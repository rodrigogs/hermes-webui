...
def test_server_py_under_750_lines(cleanup_test_sessions):
    ...
    Ceiling is 760, not 750: the deep-review gate of 2026-08-14 (PR #6658)
    requires `Connection: close` emission in Handler.end_headers(), which
    lives in server.py by definition — the framing logic cannot move to api/
    without losing the BaseHTTPRequestHandler override.
    """
